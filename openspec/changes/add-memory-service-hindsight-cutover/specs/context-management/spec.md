...
### Requirement: MuninnDB vault and graph-traversal configuration
**Reason**: MuninnDB is being removed. Memory-service does not expose a `vault` concept (Hindsight uses banks, configured in memory-service startup YAML rather than per-call), and graph traversal is performed internally by Hindsight's TEMPR strategies rather than by a caller-supplied `max_hops` parameter. Any settings related to `vault`, `max_hops`, or per-call graph depth are no longer applicable.

**Migration**: Remove `vault`, `max_hops`, and `store_confidence` from `config:memory`. Remove `MUNINN_ADDR`, `MUNINN_API_KEY`, and related env vars from every consumer. Bank identity is configured on memory-service via `MEMORY_BANK_ID`; recall/retain semantics (including how deeply Hindsight traverses its internal graph) are not consumer-tunable in this proposal.
