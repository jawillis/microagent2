...
Main-agent SHALL maintain a "base toolset" — the set of tool names that are always visible to the model regardless of which skill (if any) is active. The base toolset SHALL consist of every built-in tool registered at startup (`list_skills`, `read_skill`, `read_skill_file`, `current_time`) plus every MCP-sourced tool registered via the MCP manager (names beginning `mcp__<server>__`).
